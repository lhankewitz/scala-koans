...
class AboutLiterals extends KoanSuite with Matchers {
  koan("Integer Literals are 32-bit and can be created from decimal, octal, or hexadecimal") {
    ...
    // octal literals are removed from 2.11 see SI-7618
    //    val d = 077
    //    val i = -077
  ...
  koan(
    """Trick: To get a string representation of the
      |   float or double literal add a space after the literal""") {
    3.0.toString should be("3.0")
    3.toString should be("3")
    // (3. toString) should be("3.0") // no longer supported dot has to follow a digit to be a double/float
    (3.0 toString) should be("3.0")
  ...
  koan(
    """Character Literals can either be an a single character,
    ...
    // octal numbers no longer supported in scala 2.11 see SI-7618
  ...
  koan(
    """Multiline String literals on subsequent lines can have | to specify
